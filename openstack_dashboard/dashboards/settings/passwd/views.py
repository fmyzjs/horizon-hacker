...
def set_passwd(request):
    ...
    if(len(pass1)>=6 and pass1==pass2 and request.user.username!='admin'):
        ...
        cmd='/usr/bin/keystone --os_tenant_name=%s --os_username=%s --os_password=%s --os_auth_url=%s  user-password-update --pass=%s %s' % (keystone_cfg['admin_tenant_name'],keystone_cfg['admin_user'],keystone_cfg['admin_password'],keystone_cfg['auth_host'],pass1,request.user.id)
        os.system(cmd)
        msg=True
    else:
        msg=False
